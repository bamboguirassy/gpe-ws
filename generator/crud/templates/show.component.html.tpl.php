...
<p-fieldset *ngIf="'<?= $entity_class_name ?>'|showable" legend="Affichage des informations - Utilisateur" [toggleable]="true">
  <p-toolbar>
    <button *ngIf="'<?= $entity_class_name ?>'|editable" type="button" class="btn btn-outline-warning pull-right ml-1"
      [routerLink]="['/'+<?= $entity_var_singular ?>Srv.getRoutePrefix(),<?= $entity_var_singular ?>.id,'edit']"><i class="fa fa-pencil-square-o ml-1"></i>
      Modifier</button>
    <button *ngIf="'<?= $entity_class_name ?>'|clonable" [routerLink]="['/'+<?= $entity_var_singular ?>Srv.getRoutePrefix(),<?= $entity_var_singular ?>.id,'clone']" type="button"
  ...
  <p-tabView>
    <p-tabPanel header="Détails">
      <p-fieldset [legend]="<?= $entity_var_singular ?>.id">
        <table class="table">
